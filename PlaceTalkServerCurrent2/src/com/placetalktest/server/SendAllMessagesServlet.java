...
//		logger.info("in sendAllMessage doPost");
	    String json = req.getParameter("json");
	    Gson gson = new Gson();
	    Group group = gson.fromJson(json, Group.class);
	    HashSet<String> devices = group.getDevices();
	    
	    String status;
	    if (devices.isEmpty()) {
	      status = "Message ignored as there is no device registered!";
	    } else {
	      String sendGroupName = group.getGroupName();
		  String sendGroupLocation = group.getLocation();
		  int sendGroupRange = group.getRange();
		  Group sendGroup = new Group(sendGroupName, sendGroupLocation, sendGroupRange);
		  sendGroup.addMessage(group.getLatestMessage());
//		  sendGroup.setDevices(devices);
		  json = gson.toJson(sendGroup);
...
//	        String device = devices.get(0);
	        ...
	        queue.add(withUrl("/send").param(
	            SendMessageServlet.PARAMETER_DEVICE, device).param(SendMessageServlet.PARAMETER_JSON, json));
...
//	        for (String device : deviceArray) {
	        for (int i = 0; i < devices.size(); i++) {
	          counter++;
	          partialDevices.add(deviceArray[i]);
	          ...
	          if (partialSize == Datastore.MULTICAST_SIZE || counter == total) {
	            ...
	            TaskOptions taskOptions = TaskOptions.Builder
	                ...
	                .param(SendMessageServlet.PARAMETER_JSON, json)
	                .method(Method.POST);
