...
public class SendMessageServlet extends BaseServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
      throws IOException {
//	logger.severe("in SendMessage doPost");
...
//    logger.severe("regId = " + regId);
    if (regId != null) {
      sendSingleMessage(regId, req, resp);
      return;
    }
    String multicastKey = req.getParameter(PARAMETER_MULTICAST);
//    logger.severe("multicastKey = " + multicastKey);
    if (multicastKey != null) {
      sendMulticastMessage(multicastKey, req, resp);
      return;
    }
    
    // TODO: Remove if doesnt work
    String sgroup = req.getParameter(PARAMETER_JSON);
    Gson gson = new Gson();
    Group group = gson.fromJson(sgroup, Group.class);
//    ArrayList<String> devices = group.getDevices();
  ...
  private void sendSingleMessage(String regId, HttpServletRequest req, HttpServletResponse resp) {
//	    logger.severe("Sending message to device " + regId);
	    Message message = new Message.Builder().addData("json", req.getParameter("json")).build();
	    Result result;
	    try {
	    ...
	    if (result.getMessageId() != null) {
	      ...
	      if (error.equals(Constants.ERROR_NOT_REGISTERED)) {
	        // application has been removed from device - unregister it
	    	  logger.severe("sender.sendNoRetry returned in null result. About to unregister device from datastore");
  ...
  private void sendMulticastMessage(String multicastKey, HttpServletRequest req, HttpServletResponse resp) {
